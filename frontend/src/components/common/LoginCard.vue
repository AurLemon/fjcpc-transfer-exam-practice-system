...
const fetchData = async () => {
  ...
  const encryptedPassword = password.value
    ? sm2Encrypt(password.value, publicKey)
    : 'empty'
...
<template>
  <div class="view-login">
    <div class="view-login-main">
      ...
      <div class="view-login-tips">
        账户系统基于船政转轨练习系统的用户信息二次封装开发
      </div>
      <div class="view-login-form">
        <div
          class="view-login-form__input"
        ...
        <div class="view-login-form__login">
          ...
          >
            ...
            <div
              ...
              :class="{ show: loadStatus === 'loading' }"
            >
              autorenew
            </div>
            <div
              class="material-icons success"
              :class="{ show: loadStatus === 'success' }"
            >
              done
            </div>
            <div
              class="material-icons error"
              :class="{ show: loadStatus === 'error' }"
            >
              close
            </div>
          </button>
    ...
    <div class="view-login-desc">
      ...
      <ul class="view-login-desc__list">
        ...
        <li>
          不登录也可以做题，只不过做题数据会保留在本地，换浏览器就会丢失。
        </li>
        <li>
          身份证信息在前后端的存储都是密文的。前后端交互过程采用非对称加密算法
          SM2 加密，后端数据库采用 SHA256
