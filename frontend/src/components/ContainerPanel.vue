...
export default defineComponent({
  ...
  methods: {
    ...
    userLogout(): void {
      ...
      setTimeout(() => {
        ...
        this.notifyStore.addMessage(
          'success',
          `已登出（${dayjs().format('YYYY-MM-DD HH:mm:ss')}）`,
        )
      }, 1000)
    },
    getCulturalCount(): number {
      return this.questionStore.questionInfo.cultural_lesson.reduce(
        (
          sum: number,
          lesson: { subject: number; id: string; name: string; count: number },
        ) => {
  ...
  computed: {
    calculateUserProgress(): number {
      ...
      return Number(
        ((userProgress.current / userProgress.total) * 100).toFixed(2),
      )
    },
  },
  setup() {
...
<template>
  <nav
    class="page-container-panel"
    :class="{ show: cardStore.mobileShowPanel }"
  >
    <div class="container-panel-countdown">
      <div class="container-panel-header-countdown">
        <div class="container-panel-header-countdown__value">
          {{ questionStore.isGetQuestionInfo ? '-' : getCulturalCount() }}
        </div>
        <div class="container-panel-header-countdown__label">文化课题目数</div>
      </div>
      <div class="container-panel-header-countdown">
        <div class="container-panel-header-countdown__value">
          {{ questionStore.isGetQuestionInfo ? '-' : getProfessionCount() }}
        </div>
      ...
      <div class="container-panel-header-countdown">
        <div class="container-panel-header-countdown__value">
          {{
            questionStore.isGetQuestionInfo
              ? '-'
              : getCulturalCount() + getProfessionCount()
          }}
        </div>
      ...
      <div class="container-panel-header-countdown">
        <div
          ...
          v-tippy="{
            content: `${examTime()}（${isExamTimeTrust() ? '推算' : '已确定考试时间'}）`,
            placement: 'left',
          }"
        >
          {{ examCountdown() }}
        ...
        <div class="container-panel-header-countdown__label">
          ...
          <span
            ...
            v-if="isExamTimeTrust()"
            v-tippy="{
              content: '考试时间为推算，船政公布考试时间后会自动更新。',
            }"
            >info</span
          >
        </div>
    ...
    <div class="container-panel-status">
      ...
      >
        ...
        <div class="container-panel-status__progressWrapper">
          <div
            class="container-panel-status__progressScroll"
    ...
    <div class="container-panel-profile">
      <div
        ...
        :class="{ blur: !userStore.login.isLogged || authStore.isLoading }"
      >
        <div
          class="container-panel-profile__greeting"
          v-if="
        ...
        >
          {{
            userStore.login.isLogged && userStore.profile.name.length > 0
              ? `${userStore.profile.name[0]}同学，你好。`
              : '-'
          }}
        </div>
        <div class="container-panel-profile__greeting" v-else>
          {{
            userStore.login.isLogged && userStore.profile.nick.length > 0
              ? `${userStore.profile.nick}，你好。`
              : '-'
          }}
        </div>
        <div class="container-panel-profile__inner">
          <div
            class="container-panel-profile__account"
            v-if="userStore.profile.id_number"
          >
            {{
              userStore.login.isLogged && userStore.profile.id_number.length > 0
                ? userStore.setting.show_name === 'id_number'
                  ? userStore.profile.id_number
                  : `${userStore.profile.id_number[0]}*****************`
                : '-'
            }}
          </div>
          <div class="container-panel-profile__account" v-else>
            {{
              userStore.login.isLogged && userStore.profile.uuid.length > 0
          ...
          <div class="container-panel-profile__buttons">
            <div
              class="container-panel-profile__button container-panel-profile__edit"
              @click="openLoginCard"
              v-tippy="{ content: '切换账户' }"
            >
              <span class="material-icons">edit</span>
            </div>
            <div
              class="container-panel-profile__button container-panel-profile__logout"
              @click="userLogout"
              v-tippy="{ content: '退出' }"
            >
        ...
        <div
          class="container-panel-profile__detail"
          v-if="userStore.profile.profession && userStore.profile.school"
        >
          {{
            userStore.login.isLogged &&
            userStore.profile.profession.length > 0 &&
            userStore.profile.school.length > 0
