...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    readLogin() {
      ...
      if (
        authStore.readToken() !== null &&
        authStore.readRefreshToken() !== null
      ) {
        return true
      }
      return false
    ...
    async fetchUserProgress() {
      ...
      try {
        ...
        if (response.data.code === 200) {
          ...
          if (userStore.setting.auto_sync_data) {
            ...
            if (currentData.length > data.length) {
              ...
              const extraPids = localPids.filter(
                (pid) => !serverPids.includes(pid),
              )
          ...
          if (response.data.data.type === 'expiry_token') {
            ...
            response.data.data.type === 'token_not_exist' &&
            !this.login.refreshing
          ) {
            this.login.isLogged = false
          }
        }
        ...
        console.error(
          'Catch error in UserStore - fetchUserProgress(). Details: ',
          err,
        )
    ...
    async getAllProgress() {
      try {
        ...
        console.error(
          'Catch error in UserStore - getAllProgress(). Details: ',
          err,
        )
        return []
      }
    ...
    async getProgressSubject(course: number, subject: number, type: number) {
      try {
        ...
        const filteredProgress = progress.filter(
          (item) =>
            item.course === course &&
            (subject === -1 || item.subject === subject) &&
            (type === -1 || item.type === type),
        )
        return filteredProgress
      } catch (err) {
        console.error(
          'Catch error in UserStore - getProgressSubject(). Details: ',
          err,
        )
        return []
      }
    ...
    async addProgress(
      ...
      try {
          ...
          if (response.data.code === 200) {
            ...
            if (!serverProgress.some((item) => item.pid === pid)) {
              const updatedProgress = [
                ...serverProgress,
                { pid, course, subject, type, time: Date.now().toString() },
              ]
        ...
        console.error(
          'Catch error in UserStore - addProgress(). Details: ',
          err,
        )
        return false
      }
    ...
    async addProgressByArray(pidArray: string[]) {
      ...
      try {
        ...
        notifyStore.addMessage('failed', `添加用户进度时异常（${err}）`)
        console.error(
          'Catch error in UserStore - addProgressByArray(). Details: ',
          err,
        )
        return false
      }
    ...
    async updateProgressCount() {
      ...
      this.profile.user_progress.total =
        questionStore.getCulturalCount() + questionStore.getProfessionCount()
    },
    async fetchStarProgress() {
      ...
      try {
        ...
        console.error(
          'Catch error in UserStore - fetchStarProgress(). Details: ',
          err,
        )
        console.log(err)
      }
    ...
    async getFolderContent(folderName: string = 'wrong'): Promise<StarItem[]> {
      try {
        ...
        console.error(
          'Catch error in UserStore - getFolderContent(). Details: ',
          err,
        )
        return []
      }
    },
    async getStarSubject(
      course: number,
      subject: number,
      type: number,
      ...
      try {
        ...
        console.error(
          'Catch error in UserStore - getStarSubject(). Details: ',
          err,
        )
        return []
      }
