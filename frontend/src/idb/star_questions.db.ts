...
export async function getFolderContent(
  ...
  if (!folder) {
    console.warn(
      `Folder "${folderName}" does not exist, defaulting to "wrong".`,
    )
...
export async function addItemToFolder(
  ...
  if (!folder) {
    ...
    if (
      !folder.items.some(
        (existingItem: StarItem) => existingItem.pid === item.pid,
      )
    ) {
...
export async function checkStarExists(
  pid: string,
  folderName: string = 'wrong',
): Promise<boolean> {
  const db = await dbPromise
  ...
  const folder = await store.get(folderName)
  return folder
    ? folder.items.some((item: StarItem) => item.pid === pid)
    : false
}

export async function setStarProgress(
  starData: StarProgressData,
): Promise<void> {
  const db = await dbPromise
  const tx = (await db).transaction('star_questions', 'readwrite')
  const store = tx.objectStore('star_questions')
